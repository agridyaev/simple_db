...
def run_simple_db(simple_db_path, test_input):
...
@pytest.mark.parametrize("test_input, expected_out", UNKNOWN_COMMAND)
def test_unknown_command(request, test_input, expected_out):
    """Если пользователь ввёл неизвестную команду, то программа должна об этом сообщить,
       выведя строку «Unknown command: COMMAND»
    """
    assert run_simple_db(request.config.getoption('--simple-db-path'), test_input) == expected_out


@pytest.mark.parametrize("test_input, expected_out", DATE_WRONG_FORMAT)
def test_date_wrong_format(request, test_input, expected_out):
...
@pytest.mark.parametrize("test_input, expected_out", DATE_INVALID)
def test_date_invalid(request, test_input, expected_out):
...
@pytest.mark.parametrize("test_input, expected_out", OPERATIONS)
def test_operations(request, test_input, expected_out):
    assert run_simple_db(request.config.getoption('--simple-db-path'), test_input).rstrip() == expected_out
